...
static int AKECS_TransRBuff(char *rbuf, int size)
{
    ...
    if (!atomic_read(&data_ready)) {
        if (!atomic_read(&suspend_flag)) {
            /* skip log output for saving flash */
            //printk(KERN_ERR
...
int et_execEvent(STATE_NUM state, EVENT_NUM event)
{
    ...
    if(ret < 0)
    {
        ...
        /* return slct1, slct2 to old state */
        /* slct1 */
        if(state_table[state].slct1_peen == SLCT1_PEEN_OFF)
        {
            et_pedometerOFF();
...
#if SENSOR_6AXIS_DEBUG
        AKM_get_state(next_state); /* (after)Show register state */
#endif

    return next_state;
}
/* Pedometer 2011.03.23 J.Hirano End */
